...
def init_db():
    ...
    try:
        ...
        pass
    try:
        con.execute("ALTER TABLE shop_items ADD COLUMN staple_type TEXT DEFAULT ''")
        con.commit()
    except sqlite3.OperationalError:
        pass
    try:
...
def _raw_to_shop_item(raw: dict, city_size: str = "Town", wealth: str = "Average") -> dict:
    ...
    staple_type  = ("staple" if raw.get("Staple", "").strip()
                    else "semi" if raw.get("Semi-Staple", "").strip()
                    else "")
    return {
...
def generate_shop_items(
    ...
    for raw in staple_pool:
        if raw["Name"] not in existing_names and tag_match(raw):
            item = _raw_to_shop_item(raw, city_size, wealth)
            item["staple_type"] = "staple"
            result.append(item)
...
class ShopApp(tk.Tk):
    def __init__(self):
        ...
        try:
            ...
            saved_staple_indicators  = _prefs.get("show_staple_indicators",  "1")
        except Exception:
            pass
        ...
        self.show_staple_indicators = tk.BooleanVar(value=(saved_staple_indicators == "1"))
        ...
        try:
            ...
            con.executemany(
                ...
                [
                    ...
                    ("show_staple_indicators",  str(int(self.show_staple_indicators.get()))),
                ]
            )
            con.commit()
            con.close()
    ...
    def _populate_table(self, items: list[dict]):
        ...
        show_indicators = self.show_staple_indicators.get()
        ...
        for row_idx, item in enumerate(visible):
            ...
            # Staple indicator prefix (display only — iid uses raw name)
            staple_type = item.get("staple_type", "")
            if show_indicators and staple_type == "staple":
                name_disp = f"✦  {item['name']}"
            elif show_indicators and staple_type == "semi":
                name_disp = f"✧  {item['name']}"
            else:
                name_disp = item["name"]
    ...
    def _open_app_settings_window(self):
        ...
        _staple_ind_var = tk.BooleanVar(value=self.show_staple_indicators.get())
        tk.Checkbutton(body, text="Show staple indicators (✦ / ✧)",
                       variable=_staple_ind_var,
                       bg=c["hdr"], fg=c["fg"], selectcolor=c["sel"],
                       activebackground=c["hdr"], activeforeground=c["accent"],
                       font=("Georgia", 9)).pack(anchor="w", padx=pad, pady=2)
        ...
        def _save_app_settings():
            ...
            self._update_display_columns()
            self.show_staple_indicators.set(_staple_ind_var.get())
            self._populate_table(self.current_items)
            self._save_prefs()
            ...
            for item in self.current_items:
                cur.execute("""INSERT INTO shop_items
                    (shop_id,item_id,name,rarity,item_type,source,page,
                     ...
                     sane_cost,market_price,staple_type)
                    VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)""",
